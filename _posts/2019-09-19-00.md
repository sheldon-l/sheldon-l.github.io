...
- 浏览器自行选择以下浏览器，也可以全部都装，不冲突：
  - Chromium
    - 推荐理由：适应性优于 Firefox 和 Chrome； 开源，无痛对接 Chrome 等浏览器，并且轻巧快速；已知缺陷：数据科学工具 Jupyter 打不开；
  - Firefox 
...
- 进入第一步中下载并解压所得的文件夹，其中有一个名为 `config.json` 的配置文件， 下面的操作是为了修改这份配置文件：
  ...
  - 打开同一文件夹下的 `v2ray.exe` 的可执行文件， 终端显示 ‘...started’ 表示已经打开；
...
- 打开后还不能上网，因为该协议需要借助本地 `socks5` 中转加密:
  ...
  - 手动配置代理:
    ...
    - 取消 `对所有协议均使用相同的代理服务器` 的勾选, 若该页面已填写字段, 则将所有字段清空, 再在 `socks(win10中文版可能被翻译成'套接字')` 栏输入地址 `127.0.0.1` 和端口 `10808`，点击确定;

    - 若需关闭, 则取消上述 `代理服务器` 模块中 `为LAN使用代理服务器` 的勾选.

- 每次开机都需要打开 `v2ray.dmg` 或者 `v2rayN.exe` 若需关闭 v2ray, 从任务栏退出即可.
...
**最后：**
- 打开后还不能上网，因为该协议需要借助本地 `socks5` 中转加密:
  - `设置` - `网络` - `高级` 按钮 - `代理` 选项卡 - 勾选`socks5`后在右侧输入地址 `127.0.0.1`和端口 `10808`，确定并应用;
...
- 下载第二步中提供的 `*.json` 文件, 替换 `/etc/v2ray/config.json`, 启动， 操作如下：
    ...
    # 其他操作见官网文档
    ```
- 设置-网络-代理，选择手动配置 `socks host`， 输入地址 `127.0.0.1`和端口 `10808`，确定并应用
...
- **本地时间务必与互联网时间同步**.
- **如果电脑用的是固定 IP，有可能会被探测到异常境外流量而遭到封锁**.
- **不支持部分流媒体（Netflix, Spotify 等），不支持BT下载**.
- for such as Linux, python pip, try: 
    ```bash
...
- 若仅需 `浏览器代理`, 则可在关闭系统 `socks5` 时打开火狐浏览器的 `socks5`:
  ...
  - 打开Firefox, 点击右上角下拉菜单（三条横线）, 点击 `选项(齿轮图标)`, 在选项页搜索 `代理(英文为:proxy)`, 出现 `网络设置(network setting)`, 点选 `手动设置代理(Manual proxy configuration)`, 填写:
    - SOCKS Host: 127.0.0.1; Port: 10808; 勾选 `SOCKS v5` 和 `Proxy DNS when using SOCKS v5` (若窗口小, 显示不全, 请使用滚轮)
  - 保存并关闭即可
...
- Before connection: 
  ...
  - Proxy:
    - Set Proxy if Use V2ray: Socks 5, address is 127.0.0.1, port should be consistant with above, say '10808' here.
...
- 更稳定的服务：商用的分布式 VPN （抗封锁）
  - [Squirrel](https://www.squirrelvpn.com/), 3天试用，拉会员最多30天，下载链接已经被墙
  - [BitVPN](https://www.bitvpn.net/)， 30天试用，下载链接尚未被墙
...
- 检查 v2ray 和 socks5 是否已经打开 （未写入开机自启服务的应用，每次开机都要手动打开的）
...
2. 暂无自动测试脚本，需要手动检测，输入以下命令：
    ...
    # 正常，服务器地址未被屏蔽
        # 但连 bing.com 都打不开， 需要检查 socks5 配置
        # 打得开 bing.com 但打不开 youtube.com， DNS 污染
    # 异常，需要检查 v2ray 配置
    
    ``` 

- 检查 V2ray 和 Socks5 配置的要点：
  
1. 打开 v2ray 时终端的提示，应当为 `... started`，打开境外网站时，也会有相应提示，可参考
2. 是否正确手动设置了 socks5 代理，若用户为作修改，默认情况下都是 `127.0.0.1:10808`
3. 打开 v2ray 所在文件夹下一个名为 `config.json` 的配置文件, 是否有损坏, 同时检查里面 `outbounds` 标签下的 `user` 里的 `adress`， `port`， `id`, `alterId` （参考该线路）
